...
/////////////////////////////////////////////////////////////////////////////
// CWTFToolBarColorButton

class CWTFToolBarColorButton : public CMFCToolBarButton
{
	friend class CWTFColorBar;

	DECLARE_SERIAL(CWTFToolBarColorButton)

protected:
	CWTFToolBarColorButton(COLORREF color = RGB(0, 0, 0), BOOL bIsAutomatic = FALSE, BOOL bIsOther = FALSE,
	...
	CWTFToolBarColorButton(LPCTSTR lpszColorName, BOOL bIsDocument = FALSE)
	{
	...
	virtual BOOL OnToolHitTest(const CWnd* pWnd, TOOLINFO* pTI)
	{
		...
		{
			if (!CWTFColorBar::m_ColorNames.Lookup(m_Color, str))
			{
	...
	virtual void OnChangeParentWnd(CWnd* pWndParent)
	{
		...
		m_pParentBar = DYNAMIC_DOWNCAST(CWTFColorBar, pWndParent);
	}
	...
	CWTFColorBar* m_pParentBar;
};

IMPLEMENT_SERIAL(CWTFToolBarColorButton, CMFCToolBarButton, 1)

void CWTFToolBarColorButton::OnDraw(CDC* pDC, const CRect& rect, CMFCToolBarImages* /*pImages*/, BOOL bHorz, BOOL bCustomizeMode, BOOL bHighlight, BOOL bDrawBorder, BOOL /*bGrayDisabledButtons*/)
{
	ASSERT_VALID(pDC);
	ASSERT_VALID(this);
	...
	rectFrame.DeflateRect(2, 2);//afxData.cxBorder2, afxData.cyBorder2);
	...
	if (m_bIsOther || m_bIsAutomatic || m_bIsLabel)
	{
		...
		if (m_bIsAutomatic && m_Color != (COLORREF)-1)
		{
			...
			rectColor.DeflateRect(3, 3);//afxData.cxBorder2 + 1, afxData.cyBorder2 + 1);
			...
			rectText.left = rectColor.right + 2;//afxData.cxBorder2;
		}
...
IMPLEMENT_SERIAL(CWTFColorBar, CMFCPopupMenuBar, 1)

CMap<COLORREF,COLORREF,CString, LPCTSTR> CWTFColorBar::m_ColorNames;

CWTFColorBar::CWTFColorBar()
{
	m_nNumColumns = 0;
	m_nNumRowsHorz = 0;
	m_nNumColumnsVert = 0;
...
CWTFColorBar::CWTFColorBar(const CArray<COLORREF, COLORREF>& colors, COLORREF color, LPCTSTR lpszAutoColor, LPCTSTR lpszOtherColor, LPCTSTR lpszDocColors,
		CList<COLORREF,COLORREF>& lstDocColors, int nColumns, int nRowsDockHorz, int nColDockVert, COLORREF colorAutomatic, UINT nCommandID, CWTFColorButton* pParentBtn) :
...
CWTFColorBar::CWTFColorBar(CWTFColorBar& src, UINT uiCommandID) :
...
void CWTFColorBar::AdjustLocations()
{
	...
	for (POSITION pos = m_Buttons.GetHeadPosition(); pos != NULL; i ++)
	{
		...
		{
			CWTFToolBarColorButton* pColorButton = DYNAMIC_DOWNCAST(CWTFToolBarColorButton, pButton);
...
CSize CWTFColorBar::CalcSize(BOOL bVertDock)
{
...
CSize CWTFColorBar::GetColorGridSize(BOOL bVertDock) const
//
...
int CWTFColorBar::GetExtraHeight(int nNumColumns) const
//
...
BEGIN_MESSAGE_MAP(CWTFColorBar, CMFCPopupMenuBar)
	ON_WM_CREATE()
...
/////////////////////////////////////////////////////////////////////////////
// CWTFColorBar message handlers

void CWTFColorBar::SetDocumentColors(LPCTSTR lpszCaption, CList<COLORREF,COLORREF>& lstDocColors, BOOL bShowWhenDocked)
{
...
void CWTFColorBar::ContextToSize(BOOL bSquareButtons, BOOL bCenterButtons)
{
...
void CWTFColorBar::OnNcCalcSize(BOOL bCalcValidRects, NCCALCSIZE_PARAMS FAR* lpncsp)
{
	if (m_bIsTearOff)
	{
...
int CWTFColorBar::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
...
void CWTFColorBar::Rebuild()
{
	...
	{
		InsertButton(new CWTFToolBarColorButton(m_ColorAutomatic, TRUE, FALSE, m_strAutoColor, m_ColorSelected == (COLORREF) -1));
	...
	{
		InsertButton(new CWTFToolBarColorButton(m_colors [i], FALSE, FALSE, NULL, m_ColorSelected == m_colors [i]));
	...
	{
		InsertSeparator();
		InsertButton(new CWTFToolBarColorButton(m_strDocColors, TRUE)); // Label

		for (POSITION pos = m_lstDocColors.GetHeadPosition(); pos != NULL;)
		{
			COLORREF color = m_lstDocColors.GetNext(pos);
			InsertButton(new CWTFToolBarColorButton(color, FALSE, FALSE, NULL, !bAlreadySelected && m_ColorSelected == color, TRUE));
		}
	}
	...
	{
		InsertSeparator();
		InsertButton(new CWTFToolBarColorButton((COLORREF) -1,FALSE, TRUE, m_strOtherColor));
		InsertButton(new CWTFToolBarColorButton(m_ColorSelected, FALSE, FALSE, NULL, !bAlreadySelected, FALSE, TRUE));
	}
}

class CWTFColorBarCmdUI  : public CCmdUI
{
public:
	CWTFColorBarCmdUI ();
...
CWTFColorBarCmdUI ::CWTFColorBarCmdUI ()
{
	m_bEnabled = TRUE;  // assume it is enabled
}

void CWTFColorBarCmdUI ::Enable(BOOL bOn)
{
	m_bEnabled = bOn;
	m_bEnableChanged = TRUE;
}

void CWTFColorBar::OnUpdateCmdUI(CFrameWnd* pTarget, BOOL bDisableIfNoHndler)
{
	ASSERT_VALID(this);
	...
	if (bIsEnabled != m_bIsEnabled)
	{
		...
		{
			CWTFToolBarColorButton* pColorButton = DYNAMIC_DOWNCAST(CWTFToolBarColorButton, m_Buttons.GetNext(pos));
			if (pColorButton != NULL)
			{
...
void CWTFColorBar::DoPaint(CDC* pDC)
{
...
BOOL CWTFColorBar::OnQueryNewPalette()
{
	Invalidate();
	UpdateWindow();
...
void CWTFColorBar::OnPaletteChanged(CWnd* pFocusWnd)
{
...
BOOL CWTFColorBar::OnSendCommand(const CMFCToolBarButton* pButton)
{
...
#if 0 //KES
			if (theApp.GetFrame() != NULL)// AFXGetTopLevelFrame(this) != NULL)
			{
...
BOOL CWTFColorBar::Create(CWnd* pParentWnd, DWORD dwStyle, UINT nID, CPalette* pPalette/* = NULL*/, int nColumns/* = 0*/, int nRowsDockHorz/* = 0*/, int nColDockVert/* = 0*/)
{
...
BOOL CWTFColorBar::CreateControl(CWnd* pParentWnd, const CRect& rect, UINT nID, int nColumns, CPalette* pPalette/* = NULL*/)
{
...
int __stdcall CWTFColorBar::InitColors(CPalette* pPalette, CArray<COLORREF, COLORREF>& arColors)
{
...
void CWTFColorBar::Serialize(CArchive& ar)
{
...
void CWTFColorBar::ShowCommandMessageString(UINT /*uiCmdId*/)
{
	GetOwner()->SendMessage(WM_SETMESSAGESTRING, m_nCommandID == (UINT) -1 ? AFX_IDS_IDLEMESSAGE :(WPARAM) m_nCommandID);
}

BOOL CWTFColorBar::OpenColorDialog(const COLORREF colorDefault, COLORREF& colorRes)
{
	CMFCColorMenuButton* pColorMenuButton = NULL;
...
void CWTFColorBar::EnableAutomaticButton(LPCTSTR lpszLabel, COLORREF colorAutomatic, BOOL bEnable)
{
...
void CWTFColorBar::EnableOtherButton(LPCTSTR lpszLabel, BOOL bAltColorDlg, BOOL bEnable)
{
...
void CWTFColorBar::SetColor(COLORREF color)
{
...
void CWTFColorBar::OnMouseMove(UINT nFlags, CPoint point)
{
...
void CWTFColorBar::OnLButtonDown(UINT nFlags, CPoint point)
{
	if (IsCustomizeMode() && !m_bInternal)
	{
		return;
...
void CWTFColorBar::OnLButtonUp(UINT nFlags, CPoint point)
{
	if (!IsCustomizeMode() || m_bInternal)
	{
...
void CWTFColorBar::OnLButtonDblClk(UINT nFlags, CPoint point)
{
	if (!IsCustomizeMode() || m_bInternal)
	{
...
BOOL CWTFColorBar::PreTranslateMessage(MSG* pMsg)
{
	if ((m_pParentBtn != NULL || m_pWndPropList != NULL || m_pParentRibbonBtn != NULL)
		&& !m_bInCommand)
	{
...
BOOL CWTFColorBar::OnKey(UINT nChar)
{
...
void CWTFColorBar::OnChangeHot(int iHot)
{
	...
	if (m_pParentRibbonBtn != NULL)
	{
		...
		CWTFToolBarColorButton* pColorButton = DYNAMIC_DOWNCAST(CWTFToolBarColorButton, GetButton(iHot));
...
void CWTFColorBar::OnMouseLeave()
{
	if (m_pParentBtn == NULL && m_pWndPropList == NULL)
	{
...
BOOL __stdcall CWTFColorBar::CreatePalette(const CArray<COLORREF, COLORREF>& arColors, CPalette& palette)
{
...
CPalette* CWTFColorBar::SelectPalette(CDC* pDC)
{
	ASSERT_VALID(pDC);
...
void CWTFColorBar::SetVertMargin(int nVertMargin)
{
	ASSERT_VALID(this);
...
void CWTFColorBar::SetHorzMargin(int nHorzMargin)
{
	ASSERT_VALID(this);
...
COLORREF CWTFColorBar::GetHighlightedColor() const
{
	ASSERT_VALID(this);
	...
	CWTFToolBarColorButton* pColorButton = DYNAMIC_DOWNCAST(CWTFToolBarColorButton, GetButton(m_iHot));
