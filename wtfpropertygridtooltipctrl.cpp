...
/////////////////////////////////////////////////////////////////////////////
// CWTFPropertyGridToolTipCtrl

IMPLEMENT_DYNAMIC(CWTFPropertyGridToolTipCtrl, CWnd)

CWTFPropertyGridToolTipCtrl::CWTFPropertyGridToolTipCtrl()
{
...
BEGIN_MESSAGE_MAP(CWTFPropertyGridToolTipCtrl, CWnd)
	ON_WM_ERASEBKGND()
	ON_WM_PAINT()
	ON_WM_SETFONT()
END_MESSAGE_MAP()
...
/////////////////////////////////////////////////////////////////////////////
// CWTFPropertyGridToolTipCtrl message handlers

BOOL CWTFPropertyGridToolTipCtrl::Create(CWnd* pWndParent)
{
	ASSERT_VALID(pWndParent);
	m_pWndParent = pWndParent;

	static CString m_strClassName = ::AfxRegisterWndClass(CS_SAVEBITS, ::LoadCursor(NULL, IDC_ARROW), (HBRUSH)(COLOR_BTNFACE + 1));

	return CreateEx(0, m_strClassName, _T(""), WS_POPUP, 0, 0, 0, 0, pWndParent->GetSafeHwnd(), (HMENU) NULL);
}

BOOL CWTFPropertyGridToolTipCtrl::OnEraseBkgnd(CDC* /*pDC*/)
{
	return TRUE;
}

void CWTFPropertyGridToolTipCtrl::OnPaint()
{
...
void CWTFPropertyGridToolTipCtrl::Track(CRect rect, const CString& strText)
{
	if (GetSafeHwnd() == NULL)
	{
		return;
...
void CWTFPropertyGridToolTipCtrl::Hide()
{
	if (GetSafeHwnd() != NULL)
	{
		ShowWindow(SW_HIDE);
...
void CWTFPropertyGridToolTipCtrl::Deactivate()
{
	m_strText.Empty();
	m_rectLast.SetRectEmpty();
...
void CWTFPropertyGridToolTipCtrl::OnSetFont(CFont* pFont, BOOL bRedraw)
{
...
BOOL CWTFPropertyGridToolTipCtrl::PreTranslateMessage(MSG* pMsg)
{
